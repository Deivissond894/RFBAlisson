...
const OPTION_4_RESPONSE = `Entendido. Vend. Alisson silva, entrará em contato com você em breve. Tempo médio de atendimento em até 10 minutos Por favor, aguarde.`;
...
    // Adicionando variações que podem incluir pontuação ou espaços extras após a palavra
    'ola !', 'oi !', 'bom dia !', 'boa tarde !', 'boa noite !', 'eae !', 'ei !', 'opa !', 'tudo bem !', 'como esta !',
    'olá !', 'oi !!', 'olá !!'
]);

// Palavras-chave para identificar intenções de compra/preço (NOVO)
const PRICE_KEYWORDS = [
    'quanto', 'preço', 'valor', 'custo', 'cotar', 'cotacao', 'orcamento', 'orcar'
];

const PRODUCT_KEYWORDS = [
    'eixo', 'lavadora', 'geladeira', 'fogao', 'peca', 'produto', 'modelo', 'filtro', 'motor', 'compressor', 'resistencia', 'termostato'
];
...
        .replace(/[.,\/#!$%\^&\*;:{}=\-_`~()]/g,"")
        .replace(/\s{2,}/g,""); // Garante uma string compacta para comparação

    // Verifica se o texto normalizado está na lista de saudações
    return INITIAL_GREETINGS.has(normalizedText);
}

/**
 * Normaliza o texto e verifica se contém alguma das palavras-chave fornecidas. (NOVO)
 * @param {string} text - O texto da mensagem do cliente.
 * @param {Array<string>} keywords - Um array de palavras-chave a serem procuradas.
 * @returns {boolean} - True se o texto contiver alguma das palavras-chave, False caso contrário.
 */
function containsKeywords(text, keywords) {
    ...
    // Divide o texto em palavras para evitar correspondências parciais indesejadas
    const words = normalizedText.split(/\s+/); 

    for (const keyword of keywords) {
        // Verifica se a palavra-chave está presente como uma palavra inteira no texto
        if (words.includes(keyword.toLowerCase().trim())) {
            return true;
        }
    }
    return false;
...
        text = text.toLowerCase().trim(); // Normaliza o texto para comparação
        ...
        // Adição da lógica para saudações combinadas com intenções (NOVO)
        const hasGreeting = isInitialGreeting(text);
        const hasPriceKeyword = containsKeywords(text, PRICE_KEYWORDS);
        const hasProductKeyword = containsKeywords(text, PRODUCT_KEYWORDS);

        if (hasGreeting && (hasPriceKeyword || hasProductKeyword)) {
            // Se a mensagem contiver uma saudação E (palavra-chave de preço OU palavra-chave de produto)
            console.log(`Mensagem de ${from} contém saudação e intenção de preço/produto.`);
            await sendMessage(from, OPTION_3_RESPONSE); // Responde com a mensagem da opção 3 (consulta de preços)
            currentUserState.unrecognizedCount = 0;
            currentUserState.lastBotMessageTime = currentTime;
        }
        // 1. Verificar saudações iniciais (agora com menor prioridade ou se for apenas a saudação)
        else if (isInitialGreeting(text)) {
